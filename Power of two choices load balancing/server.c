	...
	while (!quit) {
		...
		if (n > 0) { //수신한 바이트 수가 0보다 클 때만 처리
			...
			if (RecvMsg.type == READ_REQ){ //만약 받은 요청이 읽기 요청이라면
        		...
        		strcpy(RecvMsg.value, kv[index]); //서버에 저장돼있는 value값 패킷으로 복사
			...
			sendto(sock, &RecvMsg, pkt_size, 0, (struct sockaddr *)&src_addr, sizeof(src_addr)); //클라이언트로 응답 보냄
