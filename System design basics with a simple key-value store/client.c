...
int main(int argc, char *argv[]) {
  ...
  // 서버 정보 담을 소켓 구조체 이름이 srv_addr 인거임
...
//서버측에서 클라이언트의 요청 받는 거 recvfrom 이걸로 받음
//그럼 요청 받고 나서 SendMsg.type = READ_REQ; // 요청 타입을 읽기로 선언한다 -> 이거에 대응하는 코드
//읽기요청의 타입을 읽기 응답으로 변경 -> READ_REP 이걸로 변경하면 됨
//값(value) 필드를 “DDDCCCCBBBBAAAA”로 변경할 것 -> 뒤집는 함수 만들어서 호출
//RecvMsg이게 서버측에서 수신용으로 쓸 메시지 구조체
//그럼 읽기요청의 타입은 SendMsg 구조체 타입인데 받을 땐 SendMsg 말고 RecvMsg 사용
//클라이언트에게 전송하는 건 sendto