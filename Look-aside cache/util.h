...
#define CACHE_HIT 10 //매크로 추가
#define CACHE_MISS 11 //매크로 추가
...
char* get_type(struct KVS RecvMsg){
  ...
  else if (RecvMsg.type == CACHE_HIT) type = "CACHE_HIT"; //메시지 타입 추가
  else if (RecvMsg.type == CACHE_MISS) type = "CACHE_MISS"; //메시지 타입 추가
  return type;
}
